...
import { getCourseDetails } from "../../utils/getCourseDetails";
...
export default function CoursePage() {
  ...
  const loadedCourse = useMemo(() => {
    if (!loaded_course_id || !course_code) return null;
    return dummy[0].loaded_course_tbl.find(
      (c) =>
        c.loaded_course_id === Number(loaded_course_id) &&
        c.course_code.replace(/\s+/g, "") === course_code
    );
  }, [loaded_course_id, course_code]);

  const courseDetails = useMemo(() => {
    if (!loadedCourse || !currentUserId) return null;
    return getCourseDetails(loadedCourse.loaded_course_id, currentUserId);
  }, [loadedCourse, currentUserId]);

  const filteredSections: SectionItem[] = useMemo(() => {
    if (!courseDetails) return [];
    return (courseDetails.sections ?? [])
      .filter((s) =>
        s.year_and_section.toLowerCase().includes(searchQuery.toLowerCase())
      )
      .map((s) => ({
        id: s.section_id,
