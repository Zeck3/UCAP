...
from ucap_backend.serializers.vcaa_and_vpaa import CampusCourseDetailsSerializer, CampusLoadedCourseSerializer, CampusSectionSerializer
...
def campus_course_page_view(request, loaded_course_id):
    try:
        ...
        course_details = CampusCourseDetailsSerializer(dummy_section).data

        sections = Section.objects.filter(loaded_course_id=loaded_course_id).select_related("instructor_assigned")
        sections_data = CampusSectionSerializer(sections, many=True).data
